...
public class AmazonCloudFormationCachingAgent
  ...
  public OnDemandResult handle(ProviderCache providerCache, Map<String, ?> data) {
    if (shouldHandle(data)) {
      ...
      Collection<String> keys = result.getCacheResults().get("stacks").stream()
        .map(cachedata -> cachedata.getId()).collect(Collectors.toList());
  ...
  public Collection<Map> pendingOnDemandRequests(ProviderCache providerCache) {
    ...
    Collection<String> ownedKeys = providerCache.filterIdentifiers(ON_DEMAND.getNs(), Keys.getCloudFormationKey("*", this.region, this.getAccountName()));
    Collection<Map> onDemandEntriesToReturn = providerCache.getAll(ON_DEMAND.getNs(), ownedKeys).stream()
      .map(
        cacheData -> {
  ...
  public CacheResult loadData(ProviderCache providerCache) {
    ...
    Collection<String> keys = stacks.getCacheResults().get("stacks").stream()
      .map(cachedata -> cachedata.getId()).collect(Collectors.toList());
    ...
    providerCache.getAll(ON_DEMAND.getNs(), keepInOnDemand).forEach(
      cacheData -> {
